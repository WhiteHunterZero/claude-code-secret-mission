...
# Token counts supplied by Claude Code; unavailable fields display as N/A.
$inputTokens = Get-Prop $data 'context_window.total_input_tokens'
$outputTokens = Get-Prop $data 'context_window.total_output_tokens'
$inputText = if ($null -eq $inputTokens) { 'N/A' } else { ([long]$inputTokens).ToString('N0', [Globalization.CultureInfo]::InvariantCulture) }
$outputText = if ($null -eq $outputTokens) { 'N/A' } else { ([long]$outputTokens).ToString('N0', [Globalization.CultureInfo]::InvariantCulture) }
Write-Output ("Tokens: In {0} | Out {1}" -f $inputText, $outputText)